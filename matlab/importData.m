function MAToutput = importData(subjectpath)
...
for subs = 1:iter
    ...
    for iDAQFile=1:numfiles(1) %Loop through files (or folders) in DATA/DAQ
        clearvars -except fullpath folders iter TXTpath DAQfilenames MATfilenames subjectpath DAQpath MATpath save_override_check UserName numfiles iDAQFile;
        ...
        if strcmp(DAQfilename_Extension,'.DAQ') ==1   % Check file list is in .DAQ format
            ...
            for i_DAQSubfolderFile=1:numfiles(1)
                clearvars -except iter DAQfilenames MATfilenames subjectpath DAQpath MATpath save_override_check UserName numfiles iDAQFile i_DAQSubfolderFile DAQ_subfolder_path MAT_subfolder_path DAQ_subfolder_filenames MAT_subfolder_filenames;
                MAT_file_check= 'No';
