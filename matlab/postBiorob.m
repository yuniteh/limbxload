...
for sub = 1:max(dyn_met(:,1))
    ...
    for pos = 1:4
        for ld = 1:3
            ind = dyn_met(:,1) == sub & dyn_met(:,2) == trn & dyn_met(:,3) == 1 &dyn_met(:,4) == pos & dyn_met(:,5) == ld;
            if ~isnan(nanmean(dyn_met(ind,7)))
                data = [data; sub, pos, ld, nanmean(dyn_met(ind,7))];
            end
        end
    end
end
...
% mod = fitlme(all,'acc~ld+(1|sub)','dummyvarcoding','effects')
mod = fitlme(all,'acc~pos+(1|sub)','dummyvarcoding','effects')
